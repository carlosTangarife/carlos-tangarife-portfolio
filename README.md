# 👋 Hi, I'm Carlos Tangarife

<div align="center">

### Senior Software Engineer | Legacy Modernization Expert | Cloud Architecture Specialist

**11+ years** transforming complex legacy systems into modern, scalable solutions

[![Portfolio](https://img.shields.io/badge/Portfolio-carlostangarife.com-gold?style=for-the-badge&logo=google-chrome)](https://carlostangarife.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Carlos%20Tangarife-blue?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/carlostangarife)
[![Email](https://img.shields.io/badge/Email-cjt860826@gmail.com-red?style=for-the-badge&logo=gmail)](mailto:cjt860826@gmail.com)

📍 **Manizales, Colombia** | 🌍 **Available for Remote Work**

</div>

---

## 🎯 About Me

I'm a **Senior Software Engineer** with a proven track record of transforming legacy systems into modern, scalable solutions for US-based companies. My approach combines **technical excellence** with **business understanding**, ensuring solutions are not only functional but sustainable and aligned with organizational goals.

### 💡 My Professional Philosophy

- **Code as Communication**: Writing code that speaks to both machines and humans
- **Architecture as Business Enabler**: Technical decisions driven by business value
- **Quality as Continuous Process**: Not a final step, but an ongoing commitment
- **AI-Augmented Development**: Leveraging AI tools while maintaining human expertise

---

## 🚀 Key Achievements

| Metric | Achievement |
|--------|-------------|
| **Infrastructure Reduction** | Reduced 200 AWS Lambda functions to 60 (70% reduction) |
| **Performance Improvement** | Optimized database operations from 2 days to 2 hours (96% improvement) |
| **Technical Debt** | Reduced 160 .NET legacy projects to 85 (47% reduction) |
| **Delivery Speed** | Increased development velocity by 40% through AI-assisted workflows |
| **User Impact** | Built solutions serving 3M+ active users |
| **Code Quality** | Achieved 95%+ code coverage with comprehensive test automation |

---

## 💼 Current Role

**Senior Software Engineer** at **Sombra** · Client: **CampMinder (USA)**

Leading modernization initiatives for legacy camp management platform serving diverse subscriptions with complex role-based permissions and feature flag configurations.

**Key Contributions:**
- Reduced 160 legacy .NET projects to 85 through systematic refactoring
- Architected Communication Hub (CMS platform) for multi-channel messaging via Twilio
- Pioneered AI adoption initiative increasing delivery speed by 40%
- Built internal package ecosystem with Turborepo enabling code reuse across 10+ applications

---

## 🛠️ Tech Stack

### Frontend Engineering
![React](https://img.shields.io/badge/React-Expert-61DAFB?logo=react&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-Expert-3178C6?logo=typescript&logoColor=white)
![Angular](https://img.shields.io/badge/Angular-Expert-DD0031?logo=angular&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-Proficient-4FC08D?logo=vue.js&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-Proficient-646CFF?logo=vite&logoColor=white)
![Webpack](https://img.shields.io/badge/Webpack-Proficient-8DD6F9?logo=webpack&logoColor=white)

### Backend Engineering
![Node.js](https://img.shields.io/badge/Node.js-Expert-339933?logo=node.js&logoColor=white)
![C#](https://img.shields.io/badge/C%23-Expert-239120?logo=c-sharp&logoColor=white)
![.NET Core](https://img.shields.io/badge/.NET%20Core-Expert-512BD4?logo=.net&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-Proficient-E0234E?logo=nestjs&logoColor=white)
![Clean Architecture](https://img.shields.io/badge/Clean%20Architecture-Proficient-000000?logo=architecture&logoColor=white)

### Cloud & Infrastructure
![AWS Lambda](https://img.shields.io/badge/AWS%20Lambda-Expert-FF9900?logo=aws-lambda&logoColor=white)
![AWS CloudFront](https://img.shields.io/badge/CloudFront-Expert-FF9900?logo=amazon-aws&logoColor=white)
![DynamoDB](https://img.shields.io/badge/DynamoDB-Expert-4053D6?logo=amazon-dynamodb&logoColor=white)
![Azure DevOps](https://img.shields.io/badge/Azure%20DevOps-Proficient-0078D4?logo=azure-devops&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-Proficient-2496ED?logo=docker&logoColor=white)

### Database & Data
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Expert-336791?logo=postgresql&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL%20Server-Expert-CC2927?logo=microsoft-sql-server&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-Proficient-47A248?logo=mongodb&logoColor=white)

### DevOps & Tools
![CI/CD](https://img.shields.io/badge/CI%2FCD-Proficient-000000?logo=github-actions&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-Proficient-D24939?logo=jenkins&logoColor=white)
![Git](https://img.shields.io/badge/Git-Proficient-F05032?logo=git&logoColor=white)

### AI-Assisted Development
![Cursor](https://img.shields.io/badge/Cursor-Expert-000000?logo=cursor&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-Expert-412991?logo=openai&logoColor=white)

---

## 📈 Experience Timeline

### 🏢 Sombra · CampMinder (USA)
**Senior Software Engineer** | Nov 2024 - Present
- Leading modernization initiatives for legacy camp management platform
- Reduced 160 legacy .NET projects to 85 (~47% reduction)
- Architected Communication Hub (CMS platform) for multi-channel messaging

### 🏢 Avenue Code · Blink Fitness (USA)
**Senior Full Stack Engineer** | 2022 - 2024
- Developed mission-critical mobile and web applications for nationwide gym chain (3M+ users)
- Reduced 200 AWS Lambda functions to 60 (~70% reduction) while improving performance
- Built Crowd Tracker feature for real-time gym occupancy monitoring

### 🏢 CEIBA SOFTWARE · Medellín
**Developer and Architect** | 2019 - 2021
- Evolved into client-facing technical leader for large-scale enterprise projects
- Led front-end architecture standardization across organization
- Established enterprise architectural principles (DI, IoC, SOLID)

### 🏢 XPERTGROUP · CFA
**Developer** | 2018 - 2019
- Built dynamic credit scoring platform with runtime-configurable variables
- Established DevOps automation framework with Jenkins and SonarQube
- Achieved 95%+ code coverage and reduced bug rate by 70%

---

## 🎓 Core Expertise

- **Legacy System Modernization**: Transforming old systems into modern architectures
- **Cloud Architecture**: AWS & Azure expertise, serverless solutions
- **Full-Stack Development**: React, Angular, Node.js, .NET Core
- **DevOps & CI/CD**: Automation, testing, quality assurance
- **AI-Assisted Development**: Leveraging Cursor and ChatGPT for productivity

---

## 📊 Impact Metrics

```
📉 Infrastructure:  70% reduction (200 → 60 Lambda functions)
⚡ Performance:     96% improvement (2 days → 2 hours)
🔧 Technical Debt:  47% reduction (160 → 85 projects)
🚀 Delivery Speed:  40% increase (AI-assisted workflows)
👥 Users Impacted:  3M+ active users
✅ Code Coverage:   95%+ with comprehensive automation
```

---

## 🌟 What I'm Looking For

- **Remote opportunities** (Full-time or Contract)
- **Senior/Lead Software Engineer** roles
- **Architecture & Modernization** projects
- **US-based companies** (I have extensive experience working with US clients)
- **Teams that value** technical excellence and business impact

---

## 📬 Let's Connect

I'm always open to discussing new projects, innovative ideas, or opportunities to be part of your vision.

- 🌐 **Portfolio**: [carlostangarife.com](https://carlostangarife.com)
- 💼 **LinkedIn**: [linkedin.com/in/carlostangarife](https://linkedin.com/in/carlostangarife)
- 📧 **Email**: [cjt860826@gmail.com](mailto:cjt860826@gmail.com)
- 📱 **WhatsApp**: [+57 301 229 5992](https://wa.me/573012295992)
- 📍 **Location**: Manizales, Colombia

---

## 💻 Portfolio Project

This repository contains my personal portfolio website - a premium dark & gold themed showcase of my professional journey.

**Live Site**: [carlostangarife.com](https://carlostangarife.com)

**Tech Stack**: Vanilla HTML/CSS/JS | AWS S3 + CloudFront + Route 53

**Features**:
- ✨ Premium dark & gold design
- ⚡ High performance (Lighthouse 90+)
- 📱 Fully responsive
- 🔍 SEO optimized
- 🚀 AWS infrastructure

---

<div align="center">

### ⚡ Transforming Legacy Systems into Modern Solutions

**Building software that speaks to both machines and humans**

[![GitHub](https://img.shields.io/github/followers/carlostangarife?label=Follow&style=social)](https://github.com/carlostangarife)
[![Twitter](https://img.shields.io/twitter/follow/carlostangarife?style=social)](https://twitter.com/carlostangarife)

---

⭐ **Star this repo** if you find it interesting!

**Made with precision and passion** ✨

*Last Updated: December 2025*

</div>
